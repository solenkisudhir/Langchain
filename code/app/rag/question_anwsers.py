...
def generate_response(uploaded_file, openai_api_key, query_text):
    ...
    if uploaded_file is not None:
        documents = [uploaded_file.read().decode("ISO-8859-1")]
...
with st.form('myform', clear_on_submit=True):
    openai_api_key = st.text_input('OpenAI API Key',value='', type='password', disabled=not (uploaded_file and query_text))
